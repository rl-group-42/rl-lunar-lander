...
class Agent():
    def __init__(self, env, gamma=0.99, beta=0.995, learning_rate=(0.001,), update_epoches=10, theta=0.2, dt=1e-2, sigma=0.2, batch_size=64, memory_size=1000):
        ...
        self._target_actor = Network(self._state_dims, self._action_dims, activations, learning_rate=self._learning_rate[1]).to(self._device)
        self._critic_one = Network(self._state_dims + self._action_dims, 1, learning_rate=learning_rate[2]).to(self._device)
        self._target_critic_one = Network(self._state_dims + self._action_dims, 1, learning_rate=learning_rate[3]).to(self._device)
        self._critic_two = Network(self._state_dims + self._action_dims, 1, learning_rate=learning_rate[4]).to(self._device)
        self._target_critic_two = Network(self._state_dims + self._action_dims, 1, learning_rate=learning_rate[5]).to(self._device)
    ...
    def _update_network_weights(self):
        ...
        for target_param, param in zip(self._target_critic_two.parameters(), self._critic_two.parameters()):
            # theta3 <- beta*theta2 + (1 - beta)*theta3
            target_param.data.copy_(self._beta * param.data + (1 - self._beta) * target_param.data)
...
gamma = 0.99
beta = 0.995
learning_rate = [0.001, 0.001, 0.0015, 0.0015, 0.0015, 0.0015]
