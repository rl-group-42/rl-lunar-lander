"""
This is a PyTorch implementation of the Open AI's Twin Delayed Deep Deterministic 
Policy Gradient (TD3) algorithm for continuous space.

Below follows the procedure to implement TD3 using OpenAI's Spinning Up documentation:
...
class ReplayMemory():
    """
    A replay buffer for storing experiences by a TD3 agent interacting with the environment.
...
class Network(nn.Module):
    """
    Builds a neural network. For TD3 specifically, we need 6 networks for the critic, actor 
...
class Agent():
    """
    This class is the bulk of the TD3 algorithm implementation for the agent. The main agent evnronment 
    ...
    params:
        ...
        :min_noise_clip: The lower bound of the noise when clipped
        :max_noise_clip: The upper bound of the noise when clipped
        :noise_scale: the amount of noise added to the actions during exploration
        :policy_delay: indicates how many iterations to update the neural networks
    """
    def __init__(self, env, continuity=True, train_iterations=1000, episodes=4000, epoches=100,
                 optimal_run_episodes=5, render_episode=-1, measure_after_iterations=1, measurement=True,
                 gamma=0.99, tau=0.01, learning_rate=(0.001,),
                 theta=0.2, dt=1e-2, sigma=0.2,
                 batch_size=64, memory_size=1000, update_epoches=10,
                 min_noise_clip=-0.5, max_noise_clip=0.5, noise_scale=0.2, 
                 policy_delay=2):
        ...
        # 6 neural networks. Actor, twin critics, and their target networks
        ...
        self._target_actor = Network(self._state_dims, self._action_dims, activations, 
                                     hidden_dims=hidden_layers, learning_rate=self._learning_rate[1]).to(self._device)
        self._critic_one = Network(self._state_dims + self._action_dims, 1,  
                                   hidden_dims=hidden_layers, learning_rate=learning_rate[2]).to(self._device)
        self._target_critic_one = Network(self._state_dims + self._action_dims, 1, 
                                          hidden_dims=hidden_layers, learning_rate=learning_rate[3]).to(self._device)
        self._critic_two = Network(self._state_dims + self._action_dims, 1, 
                                   hidden_dims=hidden_layers, learning_rate=learning_rate[4]).to(self._device)
        self._target_critic_two = Network(self._state_dims + self._action_dims, 1, 
                                          hidden_dims=hidden_layers, learning_rate=learning_rate[5]).to(self._device)
        ...
        self._critic_one.train()
        self._target_critic_two.train()
        self._critic_two.train()
        self._target_critic_two.train()
    ...
    def _get_target_action(self, next_state):
        """
        Computes the target actions given the next state to perform network gradient updates later.
        """
        # Convert to tensor
    ...
    def _update_network_weights(self):
        """
        Update network weights for the actor and the twin critic networks. 
        w' <- tau*w + (1 - tau)*w
        """
        ...
        # Update target critic one weights
        for target_param, param in zip(self._target_critic_one.parameters(), self._critic_one.parameters()):
            # theta2 <- tau*theta1 + (1 - tau)*theta2
            target_param.data.copy_(self._tau * param.data + (1 - self._tau) * target_param.data)

        # Update target critic two weights
        for target_param, param in zip(self._target_critic_two.parameters(), self._critic_two.parameters()):
            # theta3 <- tau*theta2 + (1 - tau)*theta3
            target_param.data.copy_(self._tau * param.data + (1 - self._tau) * target_param.data)
    ...
    def _gradient_ascent_actor(self, state):
        """
        Compute the gradient ascent update to the actor network. To later select actions which moves closer to the target,
        this will later lead to the optimal policy
        """
        # Convert to tensor
        state_tensor = self._get_tensor(state)
        ...
        # Critic value
        critic_value = self._critic_one(T.cat((state_tensor, action_tensor), 1)).flatten()
    ...
    def _learn(self):
        ...
        for epoch in range(self._update_epoches):
            ...
            # Delay actor and network weights update
